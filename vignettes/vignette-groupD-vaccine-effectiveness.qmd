...
In this project, we aim to improve the statistical efficiency of vaccine effectiveness (VE) estimation by incorporating data on the force of infection experienced by different subpopulations.
...
$$\bar{I}(t_c, a) = \sum_{a'}\Bigl[I_u^{a'}(t_c - 1) \frac{C_{a'a}^u}{N_a} + (1-\theta)\,I_v^{a'}(t_c - 1)\frac{C_{a'a}^v}{N_a}\Bigr]$$
